...
import sys
import os
# Add latex2sympy to path for local module
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'latex2sympy'))
...
def strip_string(string, skip_unit=False):
    ...
    # Normalize boxed answers (e.g., \boxed{204}) before any other processing
    if "boxed" in string:
        boxed_content = find_box(string)
        if boxed_content:
            string = boxed_content
...
def extract_answer(pred_str, data_name, use_last_number=True):
    ...
    if "final answer is $" in pred_str and "$. I hope" in pred_str:
        ...
        # NEW: Calculate LaTeX expressions to numeric values
        if any(x in pred for x in ['\\frac', '\\sqrt', '\\pi', '^', 'cdot']):
            try:
                from sympy.parsing.latex import parse_latex
                from sympy import N, simplify
                # Parse LaTeX expression
                expr = parse_latex(pred)
                # Simplify and convert to numeric
                result = simplify(expr)
                numeric_val = float(N(result))
                # Convert to integer if close to integer
                if abs(numeric_val - round(numeric_val)) < 1e-6:
                    pred = str(int(round(numeric_val)))
                else:
                    pred = str(numeric_val)
            except Exception as e:
                # If parsing fails, keep original pred
                pass
    elif "he answer is" in pred_str:
        pred = pred_str.split("he answer is")[-1].strip()
    elif "final answer is" in pred_str:
        pred = pred_str.split("final answer is")[-1].strip()
    elif re.search(r'\b(thus|so|therefore|hence)\s+answer\s+(\d+)', pred_str, re.IGNORECASE):
        # Match patterns like "Thus answer 117", "So answer 279", etc.
        match = re.search(r'\b(thus|so|therefore|hence)\s+answer\s+(\d+)', pred_str, re.IGNORECASE)
        if match:
            pred = match.group(2)
    elif re.search(r'\banswer\s+is\s+(\d+)', pred_str, re.IGNORECASE):
        # Match "answer is 117" pattern
        match = re.search(r'\banswer\s+is\s+(\d+)', pred_str, re.IGNORECASE)
        if match:
            pred = match.group(1)
    elif re.search(r'\banswer\s+(\d+)(?:\s|$|\.)', pred_str, re.IGNORECASE):
        # Match "answer 117" or "answer 279" at end of text
        match = re.search(r'\banswer\s+(\d+)(?:\s|$|\.)', pred_str, re.IGNORECASE)
        if match:
            pred = match.group(1)
    ...
    # Handle degree symbols and special formatting
    # Remove degree symbols: 336° or 336^\circ → 336
    pred = pred.replace('°', '')
    pred = pred.replace('^\\circ', '')
    pred = pred.replace('\\circ', '')
    pred = re.sub(r'\^\\?circ', '', pred)
    
    # Handle scientific notation if needed
    try:
        if '.' in pred and pred.replace('.', '').replace('-', '').isdigit():
            float_val = float(pred)
            if abs(float_val - round(float_val)) < 1e-6:
                pred = str(int(round(float_val)))
    except:
        pass
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    if "gt_cot" in example and "gt" in example:
        if data_name in ["math"]:
            ...
            gt_ans = extract_answer(example["answer"], data_name)
